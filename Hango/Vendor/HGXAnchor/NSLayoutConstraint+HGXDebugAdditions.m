//
//  NSLayoutConstraint+HGXDebugAdditions.m
//  HGXAnchor
//
...
#import "NSLayoutConstraint+HGXDebugAdditions.h"
#import "HGXConstraint.h"
#import "HGXLayoutConstraint.h"

@implementation NSLayoutConstraint (HGXDebugAdditions)
...
+ (NSDictionary *)layoutPriorityDescriptionsByValue {
    ...
    dispatch_once(&once, ^{
        ...
        descriptionMap = @{
            @(HGXLayoutPriorityDefaultHigh)      : @"high",
            @(HGXLayoutPriorityDefaultLow)       : @"low",
            @(HGXLayoutPriorityDefaultMedium)    : @"medium",
            @(HGXLayoutPriorityRequired)         : @"required",
            @(HGXLayoutPriorityFittingSizeLevel) : @"fitting size",
        };
#elif TARGET_OS_MAC
        descriptionMap = @{
            @(HGXLayoutPriorityDefaultHigh)                 : @"high",
            @(HGXLayoutPriorityDragThatCanResizeWindow)     : @"drag can resize window",
            @(HGXLayoutPriorityDefaultMedium)               : @"medium",
            @(HGXLayoutPriorityWindowSizeStayPut)           : @"window size stay put",
            @(HGXLayoutPriorityDragThatCannotResizeWindow)  : @"drag cannot resize window",
            @(HGXLayoutPriorityDefaultLow)                  : @"low",
            @(HGXLayoutPriorityFittingSizeCompression)      : @"fitting size",
            @(HGXLayoutPriorityRequired)                    : @"required",
        };
#endif
    });
...
+ (NSString *)descriptionForObject:(id)obj {
    if ([obj respondsToSelector:@selector(hgx_key)] && [obj hgx_key]) {
        return [NSString stringWithFormat:@"%@:%@", [obj class], [obj hgx_key]];
    }
...
- (NSString *)description {
    ...
    if (self.priority != HGXLayoutPriorityRequired) {
