...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        [_photoClipContainer mas_makeConstraints:^(MASConstraintMaker *make) {
...
- (void)applyPhotoFocused:(BOOL)focused animated:(BOOL)animated {
    ...
    [_photoClipContainer mas_remakeConstraints:^(MASConstraintMaker *make) {
        make.left.right.bottom.equalTo(self.contentView);
...
@implementation HangoRecordDetailViewController {
    ...
    MASConstraint *_photoRegionHeightConstraint;
}
...
- (NSArray<NSString *> *)avatarNamesForParty:(HangoParty *)party {
    ...
    for (NSString *name in party.memberAvatarNames) {
        if (names.count >= 5) {
            break;
        }
...
- (UIButton *)outlinedUploadButton {
    ...
    UIImage *icon = HangoRecordScaledIcon(@"上传照片图标", 22);
...
- (void)setupUI {
    ...
    UIImageView *fireworks = [[UIImageView alloc] initWithImage:[HangoTheme imageNamed:@"占位烟花"]];
    fireworks.contentMode = UIViewContentModeScaleAspectFit;
    [_emptyStateView addSubview:fireworks];

    UIImageView *wine = [[UIImageView alloc] initWithImage:[HangoTheme imageNamed:@"占位红酒"]];
    ...
    _dialogueButton = [self outlinedCircleButtonWithImageName:@"聊天图标三个点" iconSize:30 buttonSize:56];
    ...
    [_descWrap mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(avatars.mas_bottom).offset(16);
    ...
    [_emptyStateView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(_descWrap.mas_bottom).offset(28);
        make.left.equalTo(self.contentView).offset(36);
        make.right.equalTo(self.contentView).offset(-36);
        make.bottom.lessThanOrEqualTo(_hintLabel.mas_top).offset(-20);
    }];
    [fireworks mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.centerX.equalTo(_emptyStateView);
        make.width.lessThanOrEqualTo(_emptyStateView);
        make.height.mas_equalTo(72);
    }];
    [wine mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(fireworks.mas_bottom).offset(8);
        make.left.right.bottom.equalTo(_emptyStateView);
        make.height.mas_lessThanOrEqualTo(220);
    }];
    [_photoRegionView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(_descWrap.mas_bottom).offset(kHangoRecordPhotoCarouselTopInset);
        make.left.right.equalTo(self.contentView);
        self->_photoRegionHeightConstraint = make.height.mas_equalTo([self recordPhotoRegionHeight]);
    }];
    [_photoCarousel mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    [_hintLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.bottom.equalTo(_uploadButton.mas_top).offset(-24);
    }];
    [_uploadButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.greaterThanOrEqualTo(_photoRegionView.mas_bottom).offset(kHangoRecordPhotoRegionBottomGap);
        make.left.equalTo(self.contentView).offset(20);
        make.bottom.equalTo(self.view.mas_safeAreaLayoutGuideBottom).offset(-20);
        make.height.mas_equalTo(56);
        make.right.equalTo(_dialogueButton.mas_left).offset(-12);
    }];
