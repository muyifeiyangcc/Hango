...
@interface HangoGroupDialogueViewController () <UICollectionViewDelegateFlowLayout>
@end
...
@implementation HangoGroupDialogueViewController {
    ...
    MASConstraint *_photoCarouselHeightConstraint;
    MASConstraint *_photoCarouselTopConstraint;
...
- (void)setupUI {
    ...
    for (NSString *name in [self avatarNamesForHeader]) {
        UIImageView *img = [HangoDesignKit avatarWithName:name size:kHangoGroupHeaderAvatarSize bordered:YES];
        [img mas_makeConstraints:^(MASConstraintMaker *make) {
            make.width.height.mas_equalTo(kHangoGroupHeaderAvatarSize);
        }];
    ...
    if (remaining > 0 && [self totalMemberCount] > 5) {
        ...
        [badgeWrap mas_makeConstraints:^(MASConstraintMaker *make) {
            make.width.height.mas_equalTo(kHangoGroupHeaderAvatarSize);
        }];
        ...
        [lastAvatar mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        [_extraMemberBadge mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    _inputBar.onPhoto = ^{
        [weakSelf openDecorate];
    };
    [self.view addSubview:_inputBar];

    [_avatarStack mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.contentView).offset(8);
        make.centerX.equalTo(self.contentView);
        make.height.mas_equalTo(kHangoGroupHeaderAvatarSize);
    }];
    ...
    [_photoCarousel mas_makeConstraints:^(MASConstraintMaker *make) {
        self->_photoCarouselTopConstraint = make.top.equalTo(_avatarStack.mas_bottom).offset(0);
        make.left.right.equalTo(self.contentView);
        self->_photoCarouselHeightConstraint = make.height.mas_equalTo(0);
    }];
    [_inputBarBackground mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.right.bottom.equalTo(self.view);
        make.top.equalTo(_inputBar.mas_top).offset(-8);
    }];
    ...
    [_tableView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(_photoCarousel.mas_bottom).offset(8);
        make.left.right.equalTo(self.contentView);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString *avatarName = msg.senderAvatarName.length > 0 ? msg.senderAvatarName : msg.senderName;

    UIImageView *avatar = [HangoDesignKit avatarWithName:avatarName size:kHangoGroupDialogueAvatarSize bordered:NO];
    ...
    if (msg.itemType == HangoDialogueItemTypeImage) {
        ...
        if (outgoing) {
            [avatar mas_makeConstraints:^(MASConstraintMaker *make) {
                ...
                make.width.height.mas_equalTo(kHangoGroupDialogueAvatarSize);
            }];
