...
static const CGFloat kHangoNoMoneySheetHeight = 320.0;
...
- (void)setupUI {
    ...
    _sheet = [HangoDesignKit bottomSheetWithTitle:@"Insufficient Balance"];
    ...
    [_sheet hgx_makeConstraints:^(HGXConstraintMaker *make) {
        ...
        make.height.hgx_equalTo(kHangoNoMoneySheetHeight);
    }];
    ...
    [recharge hgx_makeConstraints:^(HGXConstraintMaker *make) {
        make.right.equalTo(_sheet).offset(-24);
        make.centerY.height.width.equalTo(cancel);
    }];

    _sheet.transform = CGAffineTransformMakeTranslation(0, kHangoNoMoneySheetHeight);
}
...
- (void)dismissBottomSheetWithCompletion:(void (^)(void))completion {
    [UIView animateWithDuration:0.25 animations:^{
        ...
        self->_sheet.transform = CGAffineTransformMakeTranslation(0, kHangoNoMoneySheetHeight);
...
- (void)rechargeTapped {
    ...
    [self dismissBottomSheetWithCompletion:^{
        if (weakSelf.onRecharge) {
            ...
            [weakSelf.navigationController pushViewController:[[HangovalueViewController alloc] init] animated:YES];
        }
    }];
}
