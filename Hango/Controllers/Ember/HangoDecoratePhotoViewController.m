...
- (void)setupUI {
    ...
    UIImage *uploadIcon = [HangoTheme imageNamed:@"upload_photo_icon"];
    ...
    [upload hgx_makeConstraints:^(HGXConstraintMaker *make) {
        make.top.equalTo(self.contentView).offset(8);
        make.right.equalTo(self.contentView).offset(-16);
        make.height.hgx_equalTo(40);
        make.width.hgx_equalTo(118);
    }];
    [_previewContainer hgx_makeConstraints:^(HGXConstraintMaker *make) {
        make.top.equalTo(upload.hgx_bottom).offset(8);
        make.left.right.equalTo(self.contentView);
        make.bottom.equalTo(panel.hgx_top);
    }];
    [_preview hgx_makeConstraints:^(HGXConstraintMaker *make) {
        make.edges.equalTo(_previewContainer);
    }];
    [_stickerCanvas hgx_makeConstraints:^(HGXConstraintMaker *make) {
        make.edges.equalTo(_previewContainer);
    }];
    [_gridOverlay hgx_makeConstraints:^(HGXConstraintMaker *make) {
        make.edges.equalTo(_previewContainer);
    }];
    [panel hgx_makeConstraints:^(HGXConstraintMaker *make) {
        make.left.right.bottom.equalTo(self.contentView);
        make.height.hgx_equalTo(170);
    }];
    [choose hgx_makeConstraints:^(HGXConstraintMaker *make) {
        make.top.equalTo(panel).offset(16);
        make.left.equalTo(panel).offset(20);
    }];
    [_decorScroll hgx_makeConstraints:^(HGXConstraintMaker *make) {
        make.top.equalTo(choose.hgx_bottom).offset(12);
        make.left.right.equalTo(panel);
        make.height.hgx_equalTo(96);
    }];
...
- (void)dismissPresentedBottomSheetWithCompletion:(void (^)(void))completion {
    UIViewController *presented = self.presentedViewController;
    if ([presented isKindOfClass:HangoPurchaseDecorViewController.class]) {
        [(HangoPurchaseDecorViewController *)presented dismissBottomSheetWithCompletion:completion];
        return;
    }
    if ([presented isKindOfClass:HangoNoMoneyViewController.class]) {
        [(HangoNoMoneyViewController *)presented dismissBottomSheetWithCompletion:completion];
        return;
    }
    if (presented) {
...
- (void)handlePurchaseForDecorIndex:(NSInteger)index {
    ...
    [[HangoRequestManager shared] requestWithDelay:0.5 inView:self.view showsHUD:YES completion:^{
        ...
        if (!success) {
            [self dismissPresentedBottomSheetWithCompletion:^{
