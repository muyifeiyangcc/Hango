...
@implementation HangoLaunchManager {
...
+ (instancetype)shared {
    static HangoLaunchManager *manager;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [[HangoLaunchManager alloc] init];
    });
    return manager;
}
...
- (void)handleApplicationDidBecomeActive {
    ...
    NSLog(@"[HangoLaunch] wireless-data permission likely granted, retry launcho after settle delay");
    _launchFetchGeneration += 1;
    _launchFetchInFlight = NO;
...
- (void)fetchLaunchEligibilityStayingOnSplashWithCompletion:(void (^)(NSDictionary * _Nullable eligibility, NSError * _Nullable error))completion {
    ...
    tryFetch = ^{
        ...
        if (elapsed >= kHangoLaunchMaxWallTime) {
            NSLog(@"[HangoLaunch] launcho wall-time timeout, route Native (A)");
            finishWithResult(nil, [NSError errorWithDomain:@"HangoLaunch"
                                                      code:-1
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(kHangoLaunchRequestWatchdog * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            NSLog(@"[HangoLaunch] launcho request watchdog fired, cancel and retry on splash");
        ...
        [[HangoRequestManager shared] preflightDNSForAPIHostWithCompletion:^(BOOL dnsResolved) {
            ...
            if (!dnsResolved) {
                ...
                if (attempt >= kHangoLaunchMaxNetworkRetries) {
                    NSLog(@"[HangoLaunch] DNS preflight exhausted, route Native (A)");
                    finishWithResult(nil, [NSError errorWithDomain:@"HangoLaunch"
                                                              code:-2
                                                          userInfo:@{NSLocalizedDescriptionKey: @"DNS lookup failed."}]);
                    return;
                }
                NSLog(@"[HangoLaunch] DNS not ready yet (attempt %ld), stay on splash", (long)attempt);
                [[HangoRequestManager shared] resetNetworkSessionForLaunchRetry];
            ...
            if ([httpSelf isStaleDNSAfterPermissionGrantError:eligibilityError]) {
                ...
                if (dnsSatisfiedFailures >= kHangoLaunchMaxDNSRetriesAfterPermission) {
                    NSLog(@"[HangoLaunch] DNS still failing after wireless-data permission, route Native (A)");
            ...
            if (canRetry) {
                ...
                if ([httpSelf isStaleDNSAfterPermissionGrantError:eligibilityError]) {
                    NSLog(@"[HangoLaunch] launcho DNS cache stale after permission (attempt %ld), refresh session", (long)attempt);
                    [[HangoRequestManager shared] resetNetworkSessionForLaunchRetry];
                    delay = kHangoLaunchDNSRetryInterval;
                } else if ([httpSelf isNetworkPermissionDeniedInError:eligibilityError]) {
                    NSLog(@"[HangoLaunch] waiting for wireless-data permission (attempt %ld), stay on splash", (long)attempt);
                } else {
                    NSLog(@"[HangoLaunch] launcho waiting for network (attempt %ld), stay on splash", (long)attempt);
                }
...
- (void)resolveLaunchDecisionWithCompletion:(void (^)(HangoLaunchDecision *))completion {
    ...
    // Before portal gate epoch: force native without calling launcho.
    if (![self hasReachedPortalGateEpoch:HangoPortalGateEpoch()]) {
        NSLog(@"[HangoLaunch] before portal gate epoch, skip launcho → Native");
    ...
    NSLog(@"[HangoLaunch] calling launcho on splash");
    [self fetchLaunchEligibilityStayingOnSplashWithCompletion:^(NSDictionary *eligibility, NSError *eligibilityError) {
        NSLog(@"[HangoLaunch] launcho response=%@ error=%@", eligibility, eligibilityError);

        HangoLaunchDecision *decision = [[HangoLaunchDecision alloc] init];

        if (eligibilityError || eligibility.count == 0) {
            NSLog(@"[HangoLaunch] launcho failed, route Native (A)");
            decision.route = HangoLaunchRouteNative;
            completion(decision);
            return;
        }
        ...
        NSLog(@"[HangoLaunch] launcho code=%@ eligible=%@", code, eligible ? @"YES" : @"NO");

        if (!eligible) {
            NSLog(@"[HangoLaunch] code != 0000, route Native (A)");
            decision.route = HangoLaunchRouteNative;
            completion(decision);
            return;
        }
        ...
        if ([self isFirstTimeLogin]) {
            NSLog(@"[HangoLaunch] code=0000, no token → Onboarding (B quick login)");
            decision.route = HangoLaunchRouteOnboarding;
        } else {
            NSLog(@"[HangoLaunch] code=0000, has token → Web (B)");
...
- (void)resolveLaunchDecisionAndApplyToWindow:(UIWindow *)window {
    ...
    if (pastPortalGate) {
        // Stay on splash with a spinner until launcho finishes.
...
- (void)enterWebInWindow:(UIWindow *)window animated:(BOOL)animated {
    HangoWebShellViewController *webShell = [[HangoWebShellViewController alloc] init];
    if (!animated) {
        window.rootViewController = webShell;
        [window makeKeyAndVisible];
        return;
    }
    [HangoAppRouter setRootViewController:webShell animated:YES];
}
...
- (void)applyLaunchDecision:(HangoLaunchDecision *)decision toWindow:(UIWindow *)window {
    switch (decision.route) {
        ...
        case HangoLaunchRouteWeb:
            window.rootViewController = [[HangoWebShellViewController alloc] init];
            [window makeKeyAndVisible];
            break;
...
- (void)completeWebEntryFromViewController:(UIViewController *)viewController
    ...
    if (![appDelegate isKindOfClass:AppDelegate.class]) {
        if (completion) {
            completion(NO, [NSError errorWithDomain:@"HangoLaunch"
                                                 code:2
    ...
    [appDelegate resolveAdjustAdidWithCompletion:^(NSString *adjustAdid) {
        ...
        NSLog(@"[HangoLaunch] login body rwkpma=%@ vpkrmn=%@", params[HangoOPILoginKeyAdjustAdid()], params[HangoOPILoginBodyKeyDeviceNo()]);
        ...
        if (token.length == 0) {
            NSError *tokenError = [NSError errorWithDomain:@"HangoLaunch"
                                                      code:1
                                                  userInfo:@{NSLocalizedDescriptionKey: @"Entry failed. Please try again."}];
            if (completion) {
